...
public class B2B_ForgotPasswordPage extends DriverTestCase
{
	@Test
	public void B2BStoreAccount() throws Exception
	{
		try{
			...
			b2b_ForgotPasswordPage_Helper = new B2B_ForgotPasswordPage_Helper(driver);
...
System.out.println("opening required website --------"+B2BStore_URL);
		...
		//	B2B_ChangePassword s = new B2B_ChangePassword();
			...
			//String gap17width= propertyReader.readApplicationFile("gap17width_B2BForgotPassword");
			b2b_ForgotPasswordPage_Helper.verifyForgotPassword(vSpace1Height,TextSpace1Height,TextSpace3Height,Gutter16Width,Width,Gutter2Width,textSpace10Height,gap17width);
			
			ExecutionLog.Log("Login into application successfully.");
		}  
		catch(Error e) 
		{
			captureScreenshot("B2B_ForgotPassword");	
